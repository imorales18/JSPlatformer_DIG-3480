...
function Level(plan) 
{
  ...
  for (var y = 0; y < this.height; y++) {
    ...
    for (var x = 0; x < this.width; x++) {
      ...
      // Use if and else to handle the cases
      //Our actors are all handled at once, they live (in a sense)-
      //-on top of the grid, and will be changing or moving. 
...
Better_Coin.prototype.type = 'better_coin';
...
//This function will check to see if our user hit an object 
//Note, we use the Math object and the .floor and .ceil methods to compare-
//-values and return if the player hits something. If out of bounds, it is a wall.
//In addtion, we want to return the field type of that position they hit if they are in bounds. 
...
Better_Coin.prototype.act = function(step) 
{
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
...
Player.prototype.moveY = function(step, level, keys) 
{
  ...
  var newPos = this.pos.plus(motion);
  var obstacle = level.obstacleAt(newPos, this.size);
  //optional mechanic. Makes you float through the clouds like Super Mario Land 2.
  if(obstacle == 'floater')
  {
    this.pos = newPos;
  } 
