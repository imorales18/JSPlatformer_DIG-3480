...
var move_speed = 8, move_dist = 2;

Goomba.prototype.act = function(step, level)
{
  var newGoo = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newGoo, this.size))
    this.pos = newGoo;
  else if (this.repeatPos)
    this.pos = this.repeatPos;
  else
...
Pimba.prototype.act = function(step, level)
{
  var newPoo = this.pos.plus(this.speed.times(step));
  if(!level.obstacleAt(newPoo, this.size))
    this.pos = newPoo;
  else if (this.repeatPos)
    this.pos = this.repeatPos;
  else 
...
Level.prototype.playerTouched = function(type, actor) 
{
  ...
  if(type == "pimba" && this.status == null)
  {
    this.status = "lost";
    this.finishDelay = 1;
  }
